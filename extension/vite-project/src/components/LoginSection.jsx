// extension/src/components/LoginSection.jsx
import React, { useState } from 'react';
import { exchangeCodeForToken } from '../services/apiService';
import { setStorageData } from '../services/storageService';
import StatusMessage from './StatusMessage';

function LoginSection({ onLogin }) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const handleGitHubLogin = async () => {
    setLoading(true);
    setError(null);

    try {
      // GitHub OAuth application client ID
      const clientId = 'YOUR_GITHUB_CLIENT_ID'; // Replace with your GitHub OAuth App client ID
      const redirectUri = chrome.identity.getRedirectURL();

      // Generate a random state value for CSRF protection
      const state = Math.random().toString(36).substring(2, 15);

      // Store state for verification
      await setStorageData({ oauth_state: state });

      // Construct the authorization URL
      const authUrl =
        `https://github.com/login/oauth/authorize?` +
        `client_id=${clientId}&` +
        `redirect_uri=${encodeURIComponent(redirectUri)}&` +
        `scope=repo&` +
        `state=${state}`;

      // Launch the auth flow
      chrome.identity.launchWebAuthFlow(
        {
          url: authUrl,
          interactive: true
        },
        async (redirectUrl) => {
          if (chrome.runtime.lastError) {
            setError('Error during authentication');
            setLoading(false);
            return;
          }

          try {
            // Parse the redirect URL
            const url = new URL(redirectUrl);
            const params = new URLSearchParams(url.search);
            const code = params.get('code');
            const returnedState = params.get('state');

            // Verify state to prevent CSRF attacks
            const { oauth_state } = await getStorageData(['oauth_state']);

            if (returnedState !== oauth_state) {
              setError('Security error: state mismatch');
              setLoading(false);
              return;
            }

            // Exchange code for token using your backend
            const tokenData = await exchangeCodeForToken(code);

            if (tokenData.access_token) {
              // Save the token
              await setStorageData({ githubToken: tokenData.access_token });

              // If user info was also returned, save it
              if (tokenData.user && tokenData.user.login) {
                await setStorageData({ repoOwner: tokenData.user.login });
              }

              setLoading(false);
              onLogin();
            } else {
              setError('Failed to get access token');
              setLoading(false);
            }
          } catch (error) {
            console.error('Authentication error:', error);
            setError(error.message || 'Error during authentication');
            setLoading(false);
          }
        }
      );
    } catch (error) {
      console.error('Login error:', error);
      setError(error.message || 'Error logging in');
      setLoading(false);
    }
  };

  return (
    <div className='flex flex-col items-center p-4'>
      <p className='text-center mb-6'>
        Connect with GitHub to sync your LeetCode solutions to a repository.
      </p>

      {error && <StatusMessage type='error' message={error} className='mb-4' />}

      <button
        onClick={handleGitHubLogin}
        disabled={loading}
        className='btn btn-primary flex items-center justify-center w-full'
      >
        {loading ? (
          <div className='animate-spin h-5 w-5 border-t-2 border-b-2 border-white rounded-full mr-2'></div>
        ) : (
          <svg className='w-5 h-5 mr-2' viewBox='0 0 24 24' fill='currentColor'>
            <path d='M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z' />
          </svg>
        )}
        Login with GitHub
      </button>
    </div>
  );
}

export default LoginSection;
